    ...
    while q:
        ...
        if obs >= rgeo_cost_obs and ore >= rgeo_cost_ore and not (did_not_build & GEO):
            ...
            q.append((time, newore - rgeo_cost_ore, newclay, newobs -
                     rgeo_cost_obs, newgeo, rore, rclay, robs, rgeo + 1, 0))
...
with advent.get_input() as fin:
    blueprints.extend(tuple(map(int, exp.findall(line))) for line in fin)
